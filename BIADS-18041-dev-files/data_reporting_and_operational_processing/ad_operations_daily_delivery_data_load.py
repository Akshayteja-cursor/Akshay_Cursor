...
def merge_order_delivery_daily_freewheel(op_oms: pd.DataFrame, fw_adj_agg: pd.DataFrame, fw_demo_agg: pd.DataFrame) -> pd.DataFrame:
    op_oms = op_oms.drop(['Advertiser ID', 'Advertiser Name'], axis=1)
    fw_adj = fw_adj_agg.reset_index()
    fw_adj['Event Month'] = fw_adj['Report End Date'].dt.to_period('M').dt.to_timestamp()
    op_fw_adj = fw_adj.merge(
        fw_demo_agg.reset_index(),
        on=['Placement ID', 'Event Month'],
        how='left',
    ).drop(columns=['Event Month']).set_index('Placement ID').join(op_oms, how='outer')
...
def daily_delivery(staq_daily_file: str, staq_daily_pm_file: str, report_date: datetime) -> pd.DataFrame:
    ...
    try:
        fw_demo_agg = read_fw_demo(ad_ops_pacing_demo, ad_ops_pacing_demo_daily, report_date, by_month=True)
        ...
        fw_demo_agg = read_fw_demo(ad_ops_pacing_demo, ad_ops_pacing_demo_daily, report_date, by_month=True, skiprows=4)
    ...
    op_fw_3p_daily = merge_order_delivery_daily_freewheel(op_oms, fw_3p_agg_daily, fw_demo_agg)
