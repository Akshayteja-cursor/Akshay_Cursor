...
def read_fw_demo(
    ...
    # One aggregate per (Placement ID, Event Month) so each month gets its own
    # monthly demo comps (BAR-style), instead of one cumulative flight comps.
    if by_month:
        fw_demo_prev = fw_demo[fw_demo['Event Month'] < demo_comp_month_start]
        fw_demo_prev_agg = fw_demo_prev.groupby(
            [fw_demo_prev.index, 'Event Month'],
        ).agg('sum', numeric_only=True)

        fw_demo_daily_cur = fw_demo_daily[
            (fw_demo_daily['Event Date'] >= demo_comp_month_start)
            & (fw_demo_daily['Event Date'] <= demo_comp_date)
        ].copy()
        fw_demo_daily_cur['Event Month'] = (
            fw_demo_daily_cur['Event Date'].dt.to_period('M').dt.to_timestamp()
        )
        fw_demo_cur_agg = fw_demo_daily_cur.groupby(
            [fw_demo_daily_cur.index, 'Event Month'],
        ).agg('sum', numeric_only=True)

        fw_demo_by_month = pd.concat(
            [fw_demo_prev_agg, fw_demo_cur_agg],
        ).groupby(level=[0, 1]).sum().astype(
            {
                'On-Target Net Delivered Impressions': 'int64',
                'Gross Counted Ads (Demo)': 'int64',
            },
        )
        fw_demo_by_month.index.names = ['Placement ID', 'Event Month']
        return fw_demo_by_month
