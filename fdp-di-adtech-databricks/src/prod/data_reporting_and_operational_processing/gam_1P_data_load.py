...
def pull_gam_report(client: Any, report_id: str, start_date: datetime, end_date: datetime, output_date:datetime) -> str:
    filename = 'GAM_Report' + output_date.strftime('_%Y%m%d') + '.csv.gz'
...
def update_daily_breakouts(filename: str) -> list:
    global drop_bucket
    ...
    for date, df in gam.groupby('Date'):
        ...
        dates.append(dt)
        df.to_parquet(f's3://{drop_bucket}/gam/daily/dt={dt}/gam_daily_{dt}.parquet', compression='gzip', storage_options={'profile': aws_profile})
        print(f'Updated partition dt={dt}')

    return dates

def update_lifetime(partitions) -> None:
    global drop_bucket

    lifetime_file = pd.read_parquet(f's3://{drop_bucket}/processed/GAM_Lifetime_Delivery.parquet', storage_options={'profile': aws_profile})
    lifetime_file = lifetime_file.set_index(['Date', 'Line Item ID', 'Order ID'])

    updated = []

    for partition in partitions:
        latest = pd.read_parquet(f's3://{drop_bucket}/gam/daily/dt={partition}/gam_daily_{partition}.parquet', storage_options={'profile': aws_profile})
    ...
    lifetime_file.to_parquet(f's3://{drop_bucket}/processed/GAM_Lifetime_Delivery.parquet', storage_options={'profile': aws_profile})

    print(f'Updated for dates - ', partitions)

# COMMAND ----------

def gam_update_daily(gam_service_account_creds_secret_arn: str, report_id: str, start_date: datetime, end_date: datetime, output_date: datetime) -> None:
    client = get_ad_manager_client(gam_service_account_creds_secret_arn)
    curr_filename = pull_gam_report(client, report_id, start_date, end_date, output_date)
    updated_partitions = update_daily_breakouts(curr_filename)
    update_lifetime(updated_partitions)
...
if __name__ == '__main__':
    ...
    try:
        gam_update_daily(gam_service_account_creds_secret_arn, report_id, start_date, end_date, report_date)
