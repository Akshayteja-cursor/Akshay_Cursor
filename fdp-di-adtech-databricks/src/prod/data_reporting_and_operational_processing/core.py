...
def read_gam_lifetime(gam_file: str) -> pd.DataFrame:
    gam = pd.read_parquet(gam_file, storage_options={'profile': aws_profile})
    gam_base_cols = [
        'Advertiser Name',
        'Order Name',
        'Line Item Name',
    ...
    cols = [c for c in gam_base_cols if c in gam.columns]
    if 'Line Item Type' in gam.columns:
        cols.append('Line Item Type')
    gam = gam.loc[:, cols]
    if 'Line Item Type' not in gam.columns:
        gam['Line Item Type'] = ''
