...
def read_gam_lifetime(gam_file: str, sports_gam_file: str, op_oms: pd.DataFrame) -> pd.DataFrame:
    ...
    def _read_gam_parquet(path: str) -> pd.DataFrame:
        df = pd.read_parquet(path, storage_options={'profile': aws_profile})
        cols = [c for c in gam_base_cols if c in df.columns]
        if 'Line Item Type' in df.columns:
            cols.append('Line Item Type')
        df = df.loc[:, cols]
        if 'Line Item Type' not in df.columns:
            df['Line Item Type'] = ''
        return df

    gam = _read_gam_parquet(gam_file)
    ...
    gam['Line Item Type'] = gam.groupby('Line Item ID')['Line Item Type'].transform(lambda x: x.dropna().iloc[-1] if not x.dropna().empty else '')
    ...
    sports_gam = _read_gam_parquet(sports_gam_file)
    ...
    sports_gam['Line Item Type'] = sports_gam.groupby('Line Item ID')['Line Item Type'].transform(lambda x: x.dropna().iloc[-1] if not x.dropna().empty else '')
...
def merge_news_delivery_data(
    ...
    if 'Line Item Type' not in gam_fw_adj.columns:
        gam_fw_adj['Line Item Type'] = ''
    gam_fw_adj['Line Item Type'] = gam_fw_adj['Line Item Type'].fillna('')
