...
def read_fw_demo_by_month(
    fw_demo_monthly: str,
    fw_demo_daily_path: str,
    report_date: datetime,
    skiprows: int = 0,
) -> pd.DataFrame:
    """Return one demo-comp numerator/denominator row per Placement ID + Event Month.

    Aligns AOS daily delivery / pacing / exec summary with BAR:
    one monthly demo rate applied to overall delivery for that month.
    Completed months come from the monthly demo file; the current month uses
    daily demo MTD rolled up to a single monthly row.
    """
    report_date = datetime.combine(report_date.date(), datetime.min.time())
    ...
    fw_demo = pd.read_csv(
        ...
        parse_dates=['Event Month'],
        encoding='utf-8',
        storage_options={'profile': aws_profile},
    ).rename({'Gross Counted Ads': 'Gross Counted Ads (Demo)'}, axis=1)

    fw_demo['On-Target Net Delivered Impressions'] = (
        fw_demo['On-Target Net Delivered Impressions'].str.replace(',', '').fillna(0).astype('int64')
    )
    fw_demo['Gross Counted Ads (Demo)'] = (
        fw_demo['Gross Counted Ads (Demo)'].str.replace(',', '').fillna(0).astype('int64')
    )

    # Completed months only from monthly file (BAR-style monthly rates)
    completed = fw_demo[fw_demo['Event Month'] < current_month_start].copy()
    completed['Event Month'] = completed['Event Month'].dt.to_period('M').dt.to_timestamp()
    completed_agg = completed.groupby(['Placement ID', 'Event Month'], as_index=False).agg(
        {
            'On-Target Net Delivered Impressions': 'sum',
            'Gross Counted Ads (Demo)': 'sum',
        },
    )
    ...
    fw_demo_daily['On-Target Net Delivered Impressions'] = (
        fw_demo_daily['On-Target Net Delivered Impressions'].str.replace(',', '').fillna(0).astype('int64')
    )
    fw_demo_daily['Gross Counted Ads (Demo)'] = (
        fw_demo_daily['Gross Counted Ads (Demo)'].str.replace(',', '').fillna(0).astype('int64')
    )

    # Current month: one MTD monthly rate from daily demo (not day-varying comps)
    current_daily = fw_demo_daily[
        (fw_demo_daily['Event Date'] >= current_month_start)
        & (fw_demo_daily['Event Date'] <= report_date)
    ].copy()
    if not current_daily.empty:
        current_daily['Event Month'] = current_month_start
        current_agg = current_daily.groupby(['Placement ID', 'Event Month'], as_index=False).agg(
            {
                'On-Target Net Delivered Impressions': 'sum',
                'Gross Counted Ads (Demo)': 'sum',
            },
        ...
        current_agg = completed_agg.iloc[0:0].copy()

    demo_by_month = pd.concat([completed_agg, current_agg], ignore_index=True)
    if demo_by_month.empty:
        return demo_by_month

    return demo_by_month.astype(
        {
            'Placement ID': 'int64',
            'On-Target Net Delivered Impressions': 'int64',
            'Gross Counted Ads (Demo)': 'int64',
...
def attach_monthly_demo_to_daily(df: pd.DataFrame, demo_by_month: pd.DataFrame, date_col: str = 'Report End Date') -> pd.DataFrame:
    """Join month-level demo metrics onto daily delivery rows by Placement + month."""
    if df.empty:
        return df
    out = df.copy()
    ...
    out = out.drop(
        columns=['On-Target Net Delivered Impressions', 'Gross Counted Ads (Demo)'],
        errors='ignore',
    )
    out = out.merge(
        demo,
        on=['Placement ID', 'Event Month'],
        how='left',
    )
    out = out.fillna(
        {
...
def delivery_weighted_monthly_demo_comp(
    demo_by_month: pd.DataFrame,
    monthly_delivery: pd.DataFrame,
) -> pd.DataFrame:
    """Build placement-level Demo Comp as delivery-weighted monthly comps (BAR-aligned).

    monthly_delivery must include Placement ID, Event Month, Net Counted Ads.
    Result index is Placement ID with On-Target / Gross Demo scaled so
    demo_comp() * lifetime delivery ~= sum(monthly_comp * monthly_delivery).
    """
    if demo_by_month.empty or monthly_delivery.empty:
        return pd.DataFrame(
            columns=['On-Target Net Delivered Impressions', 'Gross Counted Ads (Demo)'],
        )
    ...
    delivery = delivery.groupby(['Placement ID', 'Event Month'], as_index=False).agg(
        {'Net Counted Ads': 'sum'},
    )
    ...
    placement = joined.groupby('Placement ID', as_index=True).agg(
        {
            'Net Counted Ads': 'sum',
            'Weighted Demo Imps': 'sum',
        },
    )
    # Encode weighted comps into On-Target/Gross so existing demo_comp() yields the same rate:
    # Demo Comp = On-Target / Gross Demo = WeightedDemoImps / NetCountedAds
    # Use Gross Demo = Net Counted Ads and On-Target = Weighted Demo Imps.
    ...
    return placement[['On-Target Net Delivered Impressions', 'Gross Counted Ads (Demo)']].astype(
        {
            'On-Target Net Delivered Impressions': 'int64',
            'Gross Counted Ads (Demo)': 'int64',
        },
...
def read_fw_analytics_lifetime(fw_analytics_filename: str, report_date: datetime, skiprows: int = 0) -> pd.DataFrame:
    ...
    float_cols = ['Creative Duration', 'Impression Goal', 'Estimated Impression Goal', 'Ad Unit Price']
...
def read_gam_lifetime(gam_file: str, sports_gam_file: str, op_oms: pd.DataFrame) -> pd.DataFrame:
    op_oms['Placement ID'] = pd.to_numeric(op_oms['Placement ID'], errors='coerce')
    gam = pd.read_parquet(
        gam_file,
        columns=[
    ...
    sports_gam['Goal Quantity'] = sports_gam.groupby('Line Item ID')['Goal Quantity'].transform(lambda x: x.dropna().iloc[-1] if not x.dropna().empty else np.nan)
    sports_gam['Contracted Quantity'] = sports_gam.groupby('Line Item ID')['Contracted Quantity'].transform(lambda x: x.dropna().iloc[-1] if not x.dropna().empty else np.nan)
    sports_gam = sports_gam.drop(columns=['Date'])

    sports_gam.loc[:, 'Rate'] = sports_gam['Rate']/1000000
    sports_gam.loc[:, 'Total Error Count'] = sports_gam['Total Error Count'].str.replace('-', '0', regex=False).astype('int64')
    sports_gam.loc[:, 'Contracted Quantity'] = sports_gam['Contracted Quantity'].str.replace('-', '0', regex=False).astype('int64')
    sports_gam.loc[:, 'Goal Quantity'] = sports_gam['Goal Quantity'].astype('str')
    sports_gam.loc[:, 'Sales Line Item ID'] = sports_gam.apply(extract_sales_line_item_id, args=('Line Item Name',), axis=1)

    sports_gam['Line Item ID'] = pd.to_numeric(sports_gam['Line Item ID'], errors='coerce')

    sports_gam.loc[:, 'Sales Line Item ID'] = sports_gam.apply(extract_sales_line_item_id, args=('Line Item Name',), axis=1)
    sports_gam = pd.merge(sports_gam, op_oms.reset_index()[['Placement ID', 'Sales Line Item ID']], left_on='Line Item ID', right_on='Placement ID', how='left', suffixes=('_x', '_y'))
    sports_gam['Sales Line Item ID'] = sports_gam['Sales Line Item ID_x'].combine_first(sports_gam['Sales Line Item ID_y'])
    sports_gam = sports_gam.drop(['Line Item ID', 'Placement ID', 'Sales Line Item ID_x', 'Sales Line Item ID_y'], axis=1)

    sports_gam = sports_gam.dropna(subset=['Sales Line Item ID'])
    sports_gam['Sales Line Item ID'] = sports_gam['Sales Line Item ID'].astype('int64')

    sports_gam = sports_gam.rename(
        {
            'Advertiser Name': 'GAM Advertiser Name',
            'Order Name': 'GAM Order Name',
            'Salesperson': 'GAM Salesperson'
    ...
    gam.loc[:, 'Instance'] = 'News'
    sports_gam.loc[:, 'Instance'] = 'Sports'

    return pd.concat([gam, sports_gam])
...
def merge_news_delivery_data(
    ...
    news_orgs = ['FOX News & Business', 'Outkick', 'TMZ and TooFab']
    news_pg_orgs = ['FOX News & Business Programmatic', 'FOX News & Business Programmatic - GAM']

    gam = pd.merge(gam, op_oms[['Sales Line Item ID', 'Invoice Organization Name']], on=['Sales Line Item ID'], how='left').reset_index(drop=True)
    gam.loc[gam['Invoice Organization Name'].isin(news_pg_orgs), 'Instance'] = 'GAM PG'
    ...
    op_fw_adj.loc[op_fw_adj['Invoice Organization Name'].isin(news_orgs), 'Instance'] = 'News'
    op_fw_adj.loc[op_fw_adj['Invoice Organization Name'].isin(news_pg_orgs), 'Instance'] = 'FW PG'
    ...
    gam_fw_adj['Order'] = gam_fw_adj['Order'].combine_first(gam_fw_adj['Sales Order Name'])
...
def property_deal(row: pd.Series) -> Union[float, str]:
    ...
    prop_type_rollup = {
        ...
        'Fox Sold Tubi': 'Sports',
        'Fox Sports Sold Tubi': 'Sports',
        'TMZ - Digital': 'News',
        'Outkick': 'News',
        'FoxWeather.com': 'News',
...
def calculate_billable(df: pd.DataFrame) -> pd.DataFrame:
    total_active_net_cost = df[~(df['Is Future Start'])].groupby(
        'Parent Sales Line Item ID',
        as_index=False,
    ).agg({'Net Cost': 'sum'}).rename({'Net Cost': 'Total Active Net Cost'}, axis=1)
    total_net_cost = df.groupby('Parent Sales Line Item ID', as_index=False).agg({'Net Cost': 'sum'}).rename({'Net Cost': 'Total Net Cost'}, axis=1)
    ...
    order_to_active_amt_df = df[~(df['Is Future Start'])].groupby('Parent Sales Line Item ID', as_index=False).apply(
    ...
    order_to_amt_df = df.groupby('Parent Sales Line Item ID', as_index=False).apply(
