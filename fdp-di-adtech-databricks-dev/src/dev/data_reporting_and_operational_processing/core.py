...
#def read_adjuster_news(adjuster_news_file: str, skiprows: int = 8) -> pd.DataFrame:
def read_adjuster_news(adjuster_news_file: str, op_oms: pd.DataFrame, skiprows: int = 0) -> pd.DataFrame:
    ...
    #int_cols = ['Impressions', 'Impressions (3rd Party)']
    for col in int_cols:
...
def read_gam_lifetime(gam_file: str, sports_gam_file: str, op_oms: pd.DataFrame) -> pd.DataFrame:
    ...
    gam = pd.read_parquet(
        ...
        filters = [('Date', '>=', pd.Timestamp(current_year, 1, 1))],   # remove this line
        storage_options={'profile': aws_profile}
    )
    ...
    sports_gam = pd.read_parquet(
        ...
        filters = [('Date', '>=', pd.Timestamp(current_year, 1, 1))],   #remove this line later
        storage_options={'profile': aws_profile}
    )
