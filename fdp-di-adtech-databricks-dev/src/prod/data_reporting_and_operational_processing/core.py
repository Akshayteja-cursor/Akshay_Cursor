...
def read_adjuster_news(adjuster_news_file: str, skiprows: int = 8) -> pd.DataFrame:
    ...
    for col in int_cols:
        adjuster_news[col] = adjuster_news[col].str.replace(',', '').fillna(0).astype('int64')
...
def read_gam_lifetime(gam_file: str) -> pd.DataFrame:
    gam = pd.read_parquet(
        gam_file,
        columns=[
            'Advertiser Name',
...
def merge_news_delivery_data(
    ...
    news_business_orgs = ['FOX News & Business', 'FOX News & Business Programmatic', 'FOX News & Business Programmatic - GAM']
...
def identify_placement_types(df: pd.DataFrame, report_date: datetime, end_of_period_date: Optional[datetime] = None) -> pd.DataFrame:
    ...
    df.loc[:, 'Is 3P'] = df['Billable Third Party Server'] != '1st Party'
    ...
    df_1p.loc[:, 'No Delivery'] = df_1p['Net Counted Ads'] == 0
    ...
    df_1p.loc[:, 'Is Non-Ad Served'] = np.where(df_1p['Production System Name'].isin(non_ad_served_production_systems),
                                                np.where(df_1p['Is 3P'] == True,
                                                         np.where(df_1p['Impressions (3rd Party)'] == 0,
                                                                  True, False),
